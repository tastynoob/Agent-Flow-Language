...
Python、TypeScript 和未来专用 DSL 都可以提供良好的 authoring 体验，但不适合各自拥有不同的运行语义。AFL 因此需要一层语言无关的 IR，作为 frontend、package、validator 和 VM 之间的共同边界。

## 2. 当前分层

```text
Python generator --------+
TypeScript generator ----+--> AFL IR --> Validator --> TypeScript VM
future AFL DSL ----------+                         --> Other VM
```

- **AFL IR**：描述 node、basic block、Agent 工作、Memory、Frag、dependency、控制流和 flow 组合；
- **Frontend**：帮助用户构造 IR，可以提供宿主语言函数、类型检查和 package API；
- **Validator**：检查语法、symbol、operand、resource dependency 和 VM policy 边界；
- **VM**：调度已经验证的 IR，并绑定 Agent、Memory、skill、MCP 和宿主脚本；
- **Package**：发布 prompt、Agent interface、flow、formatter、schema 和 capability contract。

文本形式是 AFL IR 的可读表示。VM 内部可以把它解析成 AST、图或其他结构，但内部编码不改变语言语义。
...
直接分发 Python 或 npm package 还意味着加载 flow 时执行宿主代码，不利于静态检查、能力审查和跨 VM 移植。因此宿主 generator 可以生成 AFL IR，VM 不应把 generator 源码当作 portable flow 执行。

这不妨碍 frontend 提供自然的函数、循环和模块 API。只要这些行为最终产生同一套 AFL IR，它们就属于 authoring convenience，而不是新的 VM 语义。
...
TypeScript 暂定用于第一份 reference VM，主要考虑 Node Agent 生态、类型工具、异步 I/O、CLI 和未来浏览器工具之间的组合。这个选择不表示 AFL IR 依赖 JavaScript，也不表示 Python 无法实现相同的并发语义。

Reference VM 的价值是落实和检验语义。其他语言实现只要通过同一组 conformance tests，也可以执行 AFL IR。Provider URL、API key、模型 SDK 和 MCP transport 属于 VM binding，不进入 portable IR。
...
Frontend 生成的结果应能脱离原宿主进程完成验证、分发和执行。确实需要 Python、TypeScript 或 shell 的 flow 可以使用显式 script executor；这属于受 VM policy 管理的执行边界，不是 generator 的隐式逃逸。
...
- TypeScript 实现第一份 validator 与 reference VM；
...
这些选择为当前设计提供方向，不限制后续根据实现和案例结果调整 frontend 或 VM。
